...
export async function getSelectedNodeInfo(
  ...
  const detailsPanel = page
    .locator('.node-details-panel, [role="region"]:has-text("Relevance")')
    .first()
...
export async function hasConnectedEdges(page: Page, nodeId: string): Promise<boolean> {
  const edges = await page.locator(
    `.react-flow__edge[data-source="${nodeId}"], .react-flow__edge[data-target="${nodeId}"]`,
  )
