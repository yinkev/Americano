    ...
    concepts: [
      {
        ...
        description: 'Specialized tissue that generates and conducts electrical impulses through the heart',
        category: 'anatomy',
      },
      {
      ...
      },
      {
        ...
        description: 'Chaotic electrical activity in the ventricles with loss of coordinated contractions',
        category: 'pathology',
      },
      {
        name: 'cardiac glycoside',
        description: 'Class of drugs that increase cardiac contractility and slow AV node conduction',
...
test.describe('Knowledge Graph Construction - E2E', () => {
  ...
  test('TC-005: Should detect co-occurrence relationships', async ({ page }) => {
    ...
    const relationshipCountText = await page.locator(
      'text=/\\d+ relationships? detected/'
    ).textContent();
  ...
  test('TC-006: Should render knowledge graph with nodes and edges', async ({ page }) => {
    ...
    const graphContainer = page.locator('[role="region"][aria-label="Search results graph visualization"]');
   ...
   */
  test('TC-009: Should highlight related concepts on node selection', async ({
    page,
  }) => {
    await page.goto('/');
   ...
   */
  test('TC-010: Should render graph responsively on mobile devices', async ({
    browser,
  }) => {
