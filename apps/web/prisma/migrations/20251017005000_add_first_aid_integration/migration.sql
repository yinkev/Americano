...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'conflicts') THEN
    ALTER TABLE "conflicts" ADD COLUMN IF NOT EXISTS "sourceAFirstAidId" TEXT;
    ALTER TABLE "conflicts" ADD COLUMN IF NOT EXISTS "sourceBFirstAidId" TEXT;
    BEGIN
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name='conflicts' AND column_name='sourceachunkid'
      ) THEN
        ALTER TABLE "conflicts" ALTER COLUMN "sourceAChunkId" DROP NOT NULL;
      END IF;
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name='conflicts' AND column_name='sourcebchunkid'
      ) THEN
        ALTER TABLE "conflicts" ALTER COLUMN "sourceBChunkId" DROP NOT NULL;
      END IF;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'conflict_flags') THEN
    ALTER TABLE "conflict_flags" ADD COLUMN IF NOT EXISTS "sourceAFirstAidId" TEXT;
    ALTER TABLE "conflict_flags" ADD COLUMN IF NOT EXISTS "sourceBFirstAidId" TEXT;
    BEGIN
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name='conflict_flags' AND column_name='sourceachunkid'
      ) THEN
        ALTER TABLE "conflict_flags" ALTER COLUMN "sourceAChunkId" DROP NOT NULL;
      END IF;
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name='conflict_flags' AND column_name='sourcebchunkid'
      ) THEN
