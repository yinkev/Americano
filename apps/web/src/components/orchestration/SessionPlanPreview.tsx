  ...
  if (loading) {
    return (
      <Card className="bg-white/80 backdrop-blur-md border-white/30 shadow-[0_8px_32px_rgba(31,38,135,0.1)]">
        <CardHeader>
          <CardTitle className="font-heading text-xl">Session Plan</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <Badge
                ...
                style={{
                  backgroundColor: `${getIntensityColor(plan.intensity)}/0.1`,
            ...
            <div className="relative h-24 rounded-lg overflow-hidden">
              ...
              >
                <div className="text-center px-2">
                  ...
                  <p className="text-xs" style={{ color: 'oklch(0.4 0.1 180)' }}>
                    {phases.warmUp} min
                  </p>
                </div>
              </div>
              ...
              <div
                ...
                style={{
                  ...
                  backgroundColor: 'oklch(0.75 0.12 25)',
                }}
              >
                <div className="text-center px-2">
                  <Brain className="size-6 mx-auto mb-1" style={{ color: 'oklch(0.2 0.12 25)' }} />
                  <p className="text-xs font-semibold" style={{ color: 'oklch(0.2 0.12 25)' }}>
                    Peak Focus
                  </p>
                  <p className="text-xs" style={{ color: 'oklch(0.3 0.12 25)' }}>
                    {phases.peak} min
                  </p>
                </div>
              </div>
              ...
              <div
                ...
                style={{
                  ...
                  backgroundColor: 'oklch(0.8 0.08 145)',
                }}
              >
                <div className="text-center px-2">
                  <Moon className="size-6 mx-auto mb-1" style={{ color: 'oklch(0.3 0.08 145)' }} />
                  <p className="text-xs font-semibold" style={{ color: 'oklch(0.3 0.08 145)' }}>
                    Wind-down
                  </p>
                  <p className="text-xs" style={{ color: 'oklch(0.4 0.08 145)' }}>
                    {phases.windDown} min
                  </p>
                </div>
              </div>
              ...
              {plan.breaks.breakIntervals.map((interval, idx) => (
                <TooltipProvider key={idx}>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="text-xs">
                        {plan.breaks.breakDurations[idx]}-min break at {interval} min
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
...
function ContentItemCard({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isHovered ? 'oklch(0.95 0.01 230)' : 'oklch(0.98 0.01 230)',
        borderColor: 'oklch(0.9 0.02 230)',
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        <span style={{ color: 'oklch(0.6 0.05 230)' }}>{getItemIcon(item.type)}</span>
        <span className="text-sm font-medium text-foreground">{getItemLabel(item.type)}</span>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground capitalize">{item.phase} phase</p>
      </div>
      <div className="text-right shrink-0">
        <p className="text-sm font-medium">{item.duration} min</p>
        {item.difficulty !== undefined && (
          <p className="text-xs text-muted-foreground">Difficulty: {item.difficulty}/10</p>
        )}
      </div>
    </div>
  );
