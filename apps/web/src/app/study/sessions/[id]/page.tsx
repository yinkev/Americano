...
import { Clock, BookOpen, Target, ArrowLeft, TrendingUp, Star, CheckCircle2, Share2, Download, PlayCircle, Eye } from 'lucide-react';
  ...
  const handleShareProgress = () => {
    try {
      ...
      const completionPercentage = totalObjectives > 0
        ? Math.round((objectivesCompleted / totalObjectives) * 100)
        : 100;
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b" style={{ borderColor: 'oklch(0.9 0.01 250)' }}>
                      <th className="text-left py-3 px-4 font-semibold" style={{ color: 'oklch(0.4 0.15 250)' }}>
                        Objective
                      </th>
                      <th className="text-center py-3 px-4 font-semibold" style={{ color: 'oklch(0.4 0.15 250)' }}>
                        Time Spent
                      </th>
                      <th className="text-center py-3 px-4 font-semibold" style={{ color: 'oklch(0.4 0.15 250)' }}>
                        Estimated
                        ...
                        >
                          <td className="py-4 px-4">
                            <div className="flex items-start gap-2">
                              <CheckCircle2 className="h-5 w-5 mt-0.5 flex-shrink-0" style={{ color: 'oklch(0.65 0.2 140)' }} />
                              <div>
                                <p className="text-sm font-medium" style={{ color: 'oklch(0.4 0.15 250)' }}>
                          ...
                          <td className="py-4 px-4 text-center">
                            ...
                            {timeDelta !== 0 && (
                              <span
                                ...
                                style={{
                                  color: timeDelta > 0 ? 'oklch(0.6 0.15 30)' : 'oklch(0.65 0.2 140)',
                                }}
                              >
                          ...
                          <td className="py-4 px-4 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <Star className="h-4 w-4" style={{ color: 'oklch(0.7 0.15 60)' }} fill="oklch(0.7 0.15 60)" />
                              <span className="text-sm font-medium" style={{ color: 'oklch(0.4 0.15 250)' }}>
          ...
          >
            ...
            <div className="space-y-3">
              {(() => {
                ...
                const totalActualTime = completions.reduce((sum, c) => sum + c.timeSpentMs, 0) / 60000;
                const totalEstimatedTime = session.mission?.objectives.reduce(
                  (sum, obj) => sum + obj.estimatedMinutes, 0
                ) || 0;
                const timeDeltaPercent = totalEstimatedTime > 0
                  ? Math.round(((totalActualTime - totalEstimatedTime) / totalEstimatedTime) * 100)
                  : 0;
                  ...
                  <>
                    ...
                    >
                      <p className="text-sm" style={{ color: 'oklch(0.4 0.15 250)' }}>
                        ...
                        {parseFloat(avgSelfAssessment) >= 3 && parseFloat(avgSelfAssessment) < 4 && ' - Good progress!'}
                    ...
                    {totalEstimatedTime > 0 && (
                      ...
                      >
                        <p className="text-sm" style={{ color: 'oklch(0.4 0.15 250)' }}>
                          ...
                          {timeDeltaPercent < 0 && `${Math.abs(timeDeltaPercent)}% faster than estimated`}
                        </p>
                      </div>
                    )}
                      ...
                      >
                        ...
                        <ul className="space-y-1">
                          {lowConfidenceObjectives.map((completion, index) => {
                            ...
                            return (
                              <li key={completion.objectiveId} className="text-sm" style={{ color: 'oklch(0.5 0.1 250)' }}>
                                • {missionObjective?.objective?.objective || `Objective ${index + 1}`} (confidence: {completion.confidenceRating}/5)
                              </li>
                            );
                          })}
                        </ul>
