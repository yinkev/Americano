...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!mission) {
      return NextResponse.json(errorResponse(ErrorCodes.NOT_FOUND, 'Mission not found'), { status: 404 });
    }

    // Parse objectives
    const objectives = mission.objectives as Array<{ objectiveId: string; estimatedMinutes: number }>;
    ...
    return NextResponse.json(errorResponse(ErrorCodes.INTERNAL_ERROR, 'Failed to generate mission preview recommendations'), { status: 500 });
  }
}
