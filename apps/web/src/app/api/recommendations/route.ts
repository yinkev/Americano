...
import { ContentSourceType } from '@/generated/prisma';
  ...
  try {
    ...
    if (!validatedParams.success) {
      return NextResponse.json(
        errorResponse(ErrorCodes.VALIDATION_ERROR, 'Invalid query parameters', validatedParams.error.issues),
        { status: 400 }
      );
    }
    ...
    if (contextType === 'objective') {
      ...
      if (!objective) {
        return NextResponse.json(errorResponse(ErrorCodes.NOT_FOUND, 'Objective not found'), { status: 404 });
      }
      ...
      if (!session) {
        return NextResponse.json(errorResponse(ErrorCodes.NOT_FOUND, 'Session not found'), { status: 404 });
      }
    ...
    return NextResponse.json(errorResponse(ErrorCodes.INTERNAL_ERROR, 'Failed to generate recommendations'), { status: 500 });
  }
}
