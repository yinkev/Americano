...
const applyInterventionSchema = z.object({
  ...
  metadata: z.record(z.any()).optional(), // Additional intervention-specific data
});

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { userId, interventionType, accepted, sessionId, metadata } = applyInterventionSchema.parse(body);
    ...
    switch (interventionType) {
      ...
      case 'CONTENT_SIMPLIFICATION':
        ...
        if (metadata?.sessionContext) {
          const recommendation = await difficultyAdapter.recommendContentModification(metadata.sessionContext);
          recoveryPlan = {
