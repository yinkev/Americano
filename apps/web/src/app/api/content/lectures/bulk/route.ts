...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Array.isArray(lectureIds) || lectureIds.length === 0) {
      return Response.json(
        { success: false, error: { code: 'MISSING_LECTURE_IDS', message: 'lectureIds array is required and must not be empty' } },
        { status: 400 }
      );
    }
    ...
    if (lectureIds.length > 50) {
      return Response.json(
        { success: false, error: { code: 'TOO_MANY_LECTURES', message: 'Maximum 50 lectures per bulk operation' } },
        { status: 400 }
      );
    }
    ...
    if (!action || !['move', 'tag', 'delete'].includes(action)) {
      return Response.json(
        { success: false, error: { code: 'INVALID_ACTION', message: 'Action must be one of: move, tag, delete' } },
        { status: 400 }
      );
    }
    ...
    if (lectureCount !== lectureIds.length) {
      return Response.json(
        { success: false, error: { code: 'LECTURES_NOT_FOUND', message: 'Some lectures not found or do not belong to user' } },
        { status: 404 }
      );
    }
    ...
    switch (action) {
      case 'move':
        if (!data?.courseId) {
          return Response.json(
            { success: false, error: { code: 'MISSING_COURSE_ID', message: 'courseId is required for move action' } },
            { status: 400 }
          );
        }
      ...
      case 'tag':
        if (!data?.tags || !Array.isArray(data.tags) || data.tags.length === 0) {
          return Response.json(
            { success: false, error: { code: 'MISSING_TAGS', message: 'tags array is required for tag action' } },
            { status: 400 }
          );
        }
        ...
        if (data.tags.length > 10) {
          return Response.json(
            { success: false, error: { code: 'TOO_MANY_TAGS', message: 'Maximum 10 tags allowed' } },
            { status: 400 }
          );
        }
        ...
        for (const tag of data.tags) {
          if (typeof tag !== 'string' || tag.length === 0 || tag.length > 30) {
            return Response.json(
              { success: false, error: { code: 'INVALID_TAG', message: 'Each tag must be 1-30 characters' } },
              { status: 400 }
            );
          }
        }
