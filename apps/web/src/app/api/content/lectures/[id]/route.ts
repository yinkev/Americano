  ...
  try {
    ...
    if (title !== undefined) {
      if (!title || title.trim().length === 0) {
        return Response.json(
          { success: false, error: { code: 'MISSING_TITLE', message: 'Lecture title cannot be empty' } },
          { status: 400 }
        );
      }
      ...
      if (title.length > 200) {
        return Response.json(
          { success: false, error: { code: 'TITLE_TOO_LONG', message: 'Lecture title must be 200 characters or less' } },
          { status: 400 }
        );
      }
    }
    ...
    if (topicTags !== undefined) {
      ...
      if (topicTags.length > 10) {
        return Response.json(
          { success: false, error: { code: 'TOO_MANY_TAGS', message: 'Maximum 10 tags allowed per lecture' } },
          { status: 400 }
        );
      }
