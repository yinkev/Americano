...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (sessions.length !== validatedParams.sessionIds.length) {
    ...
    throw ApiError.notFound(`Session(s) not found or not accessible: ${missingIds.join(', ')}`)
  }
  ...
  const sessionAnalytics = await Promise.all(
    sessions.map(async (session) => {
      ...
      return {
        ...
        completionRate:
          missionObjectives.length > 0 ? objectiveCompletions.length / missionObjectives.length : 0,
        ...
        averageSelfAssessment:
          analytics.objectives.length > 0
            ? analytics.objectives.reduce((sum, o) => sum + (o.selfAssessment || 0), 0) /
              analytics.objectives.length
            : null,
        averageConfidenceRating:
          analytics.objectives.length > 0
            ? analytics.objectives.reduce((sum, o) => sum + (o.confidenceRating || 0), 0) /
              analytics.objectives.length
            : null,
      }
    }),
...
function calculateImprovements(
  ...
  if (accuracyImprovement > 0.1) {
    improvements.push(`Card accuracy improved by ${(accuracyImprovement * 100).toFixed(0)}%`)
  ...
  if (first.averageSelfAssessment !== null && last.averageSelfAssessment !== null) {
    const assessmentImprovement = last.averageSelfAssessment - first.averageSelfAssessment
  ...
  if (first.averageConfidenceRating !== null && last.averageConfidenceRating !== null) {
    const confidenceImprovement = last.averageConfidenceRating - first.averageConfidenceRating
    if (confidenceImprovement > 0.5) {
      improvements.push(`Confidence increased by ${confidenceImprovement.toFixed(1)} stars`)
    }
  }
  ...
  const timePercentChange =
    first.averageObjectiveTimeMs > 0 ? (timeChange / first.averageObjectiveTimeMs) * 100 : 0
  ...
  if (cardVolumeChange > 5) {
    improvements.push(`Increased review volume by ${cardVolumeChange} cards per session`)
  }
