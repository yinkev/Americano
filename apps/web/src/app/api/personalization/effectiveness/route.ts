...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (!validMetrics.includes(metricFilter)) {
    throw ApiError.badRequest(`Invalid metric. Must be one of: ${validMetrics.join(', ')}`)
  }
  ...
  const cacheKey = `user:${user.id}:personalization:effectiveness:${startDate.toISOString()}:${endDate.toISOString()}:${metricFilter}`

  const response = await withCache(cacheKey, CACHE_TTL.MEDIUM, async () => {
    // OPTIMIZED: Fetch configs and metrics in parallel
    const [activeConfigs, effectivenessMetrics] = await Promise.all([
      ...
      prisma.personalizationEffectiveness.findMany({
        ...
        select: {
          startDate: true,
          endDate: true,
          retentionImprovement: true,
          performanceImprovement: true,
          completionRateChange: true,
    ...
    if (effectivenessMetrics.length > 0) {
      ...
      if (validMetrics.length > 0) {
        aggregateMetrics.retentionImprovement =
          validMetrics.reduce((sum, m) => sum + (m.retentionImprovement || 0), 0) /
          validMetrics.length
        aggregateMetrics.performanceImprovement =
          validMetrics.reduce((sum, m) => sum + (m.performanceImprovement || 0), 0) /
          validMetrics.length
        aggregateMetrics.completionRateChange =
          validMetrics.reduce((sum, m) => sum + (m.completionRateChange || 0), 0) /
          validMetrics.length
