...
import { validateRequest, submitChallengeSchema } from '@/lib/validation';
  ...
  try {
    ...
    if (!prompt) {
      return NextResponse.json(
        errorResponse('PROMPT_NOT_FOUND', 'Challenge prompt not found'),
        { status: 404 }
      );
    }
    ...
    const feedbackResponse = await fetch(`${pythonServiceUrl}/validation/generate-corrective-feedback`, {
      method: 'POST',
      headers: {
