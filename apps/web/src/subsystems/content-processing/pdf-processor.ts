    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`OCR service error on retry (${response.status}): ${errorText || response.statusText}`);
          }
        ...
        if (retryError instanceof Error && retryError.name === 'AbortError') {
          throw new Error('OCR service timeout after retry - file is too large or complex to process');
        }
        throw new Error(`OCR service failed after retry: ${retryError instanceof Error ? retryError.message : 'Unknown error'}`);
      }
    }
  }
