...
export interface StressIndicator {
  type: 'RESPONSE_LATENCY' | 'ERROR_RATE' | 'ENGAGEMENT_DROP' | 'PERFORMANCE_DECLINE' | 'DURATION_STRESS';
...
export class CognitiveLoadMonitor {
  ...
  detectStressIndicators(sessionData: SessionBehavioralData): StressIndicator[] {
    ...
    if (sessionData.responseLatencies.length > 0) {
      const avgLatency = sessionData.responseLatencies.reduce((a, b) => a + b, 0) / sessionData.responseLatencies.length;
      ...
      if (latencyIncrease > 0.15) {
        indicators.push({
          ...
          severity: latencyIncrease > 0.50 ? 'HIGH' : latencyIncrease > 0.30 ? 'MEDIUM' : 'LOW',
          value: avgLatency,
    ...
    if (sessionData.errorRate > 0.20) {
      indicators.push({
        type: 'ERROR_RATE',
        severity: sessionData.errorRate > 0.40 ? 'HIGH' : sessionData.errorRate > 0.30 ? 'MEDIUM' : 'LOW',
    ...
    if (sessionData.engagementMetrics) {
      const pauseRatio = sessionData.engagementMetrics.pauseDurationMs / (sessionData.sessionDuration * 60 * 1000);
      if (pauseRatio > 0.20) {
        indicators.push({
          type: 'ENGAGEMENT_DROP',
          severity: pauseRatio > 0.40 ? 'HIGH' : pauseRatio > 0.30 ? 'MEDIUM' : 'LOW',
          value: pauseRatio,
    ...
    if (sessionData.performanceScores.length >= 5) {
      ...
      const baseline = sessionData.baselineData?.baselinePerformance || 0.70;
      const decline = (baseline - recentPerf) / baseline;

      if (decline > 0.20) {
        indicators.push({
          type: 'PERFORMANCE_DECLINE',
          severity: decline > 0.40 ? 'HIGH' : decline > 0.30 ? 'MEDIUM' : 'LOW',
          value: decline,
  ...
  async recordLoadMetric(
    ...
    if (loadData.loadScore > 80) {
      await prisma.behavioralEvent.create({
        data: {
          userId,
          eventType: 'SESSION_ENDED', // Placeholder event type
          eventData: {
            sessionId,
            loadScore: loadData.loadScore,
            overload: true,
  ...
  private calculateResponseLatencyScore(data: SessionBehavioralData): number {
    ...
    const avgLatency = data.responseLatencies.reduce((a, b) => a + b, 0) / data.responseLatencies.length;
    ...
    if (increase > 0.50) return 100;
    if (increase > 0.30) return 75;
  ...
  private calculatePerformanceDeclineScore(data: SessionBehavioralData): number {
    ...
    const baseline = data.baselineData?.baselinePerformance || 0.70;
  ...
  private generateRecommendations(loadScore: number, indicators: StressIndicator[]): string[] {
    ...
    if (loadScore > 80) {
      recommendations.push('Critical cognitive overload detected - take a 10-minute break immediately');
