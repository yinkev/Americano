...
export class PredictionAccuracyTracker {
  ...
  async recordActualOutcome(
    ...
    // Create feedback record if manual override
    if (manualOverride) {
      await prisma.predictionFeedback.create({
        data: {
          predictionId,
          userId: prediction.userId,
          feedbackType: actualStruggle ? FeedbackType.HELPFUL : FeedbackType.INACCURATE,
          actualStruggle,
          comments: 'Manual outcome override by user'
        }
      });
    }
    ...
    if (currentMetrics.accuracy < 0.75 && currentMetrics.sampleSize > 30) {
      console.log(`[PredictionAccuracyTracker] Model accuracy below threshold: ${currentMetrics.accuracy.toFixed(2)}`);
   ...
   * - Validation score <60%
   */
  async captureOutcomeFromSession(
    sessionId: string,
    objectiveId: string
  ): Promise<void> {
    ...
    const objectiveReviews = session.reviews.filter(
      r => r.card.objective?.id === objectiveId
    );
    ...
    const againCount = objectiveReviews.filter(
      r => r.rating === ReviewRating.AGAIN
    ).length;

    // Criterion 3: Validation score <60%
    const objectiveValidations = session.validationResponses.filter(
      v => {
    ...
    if (objectiveValidations.length > 0) {
      avgValidationScore = objectiveValidations.reduce(
        (sum, v) => sum + v.score,
        0
  ...
  private calculateCalibration(
    ...
    for (const binRange of binRanges) {
      const binPredictions = predictions.filter(
        p => p.predictedStruggleProbability >= binRange.min &&
      ...
      const avgPredictedProbability = binPredictions.reduce(
        (sum, p) => sum + p.predictedStruggleProbability,
        0
      ) / binPredictions.length;

      const actualStruggles = binPredictions.filter(
        p => p.actualOutcome === true
      ).length;
    ...
    brierScore = predictions.reduce((sum, p) => {
  ...
  async analyzeErrorPatterns(): Promise<ErrorAnalysis> {
    ...
    const falsePositivePatterns = await this.detectErrorPatterns(
      falsePositives,
      'FALSE_POSITIVE'
    );

    // Analyze false negative patterns
    const falseNegativePatterns = await this.detectErrorPatterns(
      falseNegatives,
      'FALSE_NEGATIVE'
    );

    // Analyze feature-level errors
    const featureErrorAnalysis = this.analyzeFeatureErrors(
      falsePositives,
      falseNegatives
    );
  ...
  private async detectErrorPatterns(
    ...
    if (historicalStruggleErrors.length / errorPredictions.length > 0.4) {
      patterns.push({
        pattern: errorType === 'FALSE_POSITIVE'
        ...
        suggestedFix: errorType === 'FALSE_POSITIVE'
  ...
  private analyzeFeatureErrors(
    ...
    for (const featureName of featureNames) {
      ...
      const fpAvg = fpValues.length > 0
        ? fpValues.reduce((sum, v) => sum + v, 0) / fpValues.length
        : 0.5;
      ...
      const fnValues = falseNegatives
        .map(p => (p.featureVector as any)?.[featureName])
        .filter(v => v !== undefined);
      const fnAvg = fnValues.length > 0
        ? fnValues.reduce((sum, v) => sum + v, 0) / fnValues.length
        : 0.5;
  ...
  async generateModelImprovementPlan(): Promise<ImprovementRecommendations> {
    ...
    const needsRetraining = currentMetrics.accuracy < targetAccuracy &&
                           currentMetrics.sampleSize >= 30;
    ...
    for (const insight of errorAnalysis.featureErrorAnalysis) {
      if (Math.abs(insight.currentWeight - insight.suggestedWeight) > 0.05 &&
    ...
    for (const pattern of [...errorAnalysis.falsePositivePatterns, ...errorAnalysis.falseNegativePatterns]) {
    ...
    if (currentMetrics.precision < 0.65) {
      dataCollectionSuggestions.push({
        ...
        reasoning: 'Precision is low - too many false positives. Need user feedback to understand why predictions are wrong.',
    ...
    if (currentMetrics.recall < 0.70 && currentMetrics.sampleSize > 30) {
      architectureSuggestions.push({
