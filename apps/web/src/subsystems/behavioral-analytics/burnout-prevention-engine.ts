...
export interface WarningSignal {
  type: 'CHRONIC_OVERLOAD' | 'PERFORMANCE_DROP' | 'ENGAGEMENT_LOSS' | 'IRREGULAR_PATTERN' | 'NO_RECOVERY';
  detected: boolean;
...
export interface InterventionPlan {
  interventionType: 'MANDATORY_REST' | 'WORKLOAD_REDUCTION' | 'SCHEDULE_ADJUSTMENT' | 'CONTENT_SIMPLIFICATION';
  description: string;
...
export class BurnoutPreventionEngine {
  ...
  async assessBurnoutRisk(userId: string): Promise<BurnoutRiskAssessment> {
    ...
    try {
      ...
      const recommendations = this.generateRecommendations(riskLevel, contributingFactors, warningSignals);
  ...
  recommendIntervention(riskAssessment: BurnoutRiskAssessment): InterventionPlan {
    ...
    topFactors.forEach(factor => {
      if (factor.factor === 'Study Intensity' && !recommendedActions.some(a => a.includes('study hours'))) {
      ...
      if (factor.factor === 'Chronic Cognitive Load') {
        recommendedActions.push('💡 Persistent high load - switch to easier content and more scaffolding');
      }
      if (factor.factor === 'Performance Decline') {
        recommendedActions.push('💡 Performance dropping - review prerequisite concepts before new material');
      }
      if (factor.factor === 'Recovery Deficit') {
        recommendedActions.push('💡 Insufficient recovery - schedule 2 consecutive low-intensity days');
      }
    });
  ...
  async trackRecoveryProgress(userId: string, interventionId: string): Promise<RecoveryMetrics> {
    ...
    const recoveryProgress = baseline.riskScore > 0
  ...
  private calculateChronicLoadScore(loadMetrics: any[]): number {
    ...
    const highLoadDays = loadMetrics.filter(m => m.loadScore > 60).length;
    const totalDays = Math.max(1, new Set(loadMetrics.map(m =>
    ...
    if (highLoadRatio > 0.50) return 100;
    if (highLoadRatio > 0.35) return 75;
    if (highLoadRatio > 0.20) return 50;
    if (highLoadRatio > 0.10) return 25;
    return 0;
  }
  ...
  private calculateIrregularityScore(missions: any[]): number {
    ...
    if (missRate > 0.30) return 100;
    if (missRate > 0.20) return 75;
    if (missRate > 0.10) return 50;
  ...
  private calculateEngagementDecayScore(missions: any[], sessions: any[]): number {
    ...
    const skippedMissions = missions.filter(m => m.status === 'SKIPPED').length;
    const lowDifficultyRatings = missions.filter(m => m.difficultyRating && m.difficultyRating <= 2).length;

    const engagementIssues = skippedMissions + lowDifficultyRatings;
    const engagementRatio = engagementIssues / missions.length;

    if (engagementRatio > 0.40) return 100;
    if (engagementRatio > 0.30) return 75;
    if (engagementRatio > 0.20) return 50;
    if (engagementRatio > 0.10) return 25;
    return 0;
  }
  ...
  private generateRecommendations(
    ...
    factors.filter(f => f.severity === 'HIGH' || f.severity === 'CRITICAL').forEach(factor => {
    ...
    signals.filter(s => s.severity === 'HIGH').forEach(signal => {
  ...
  private getInterventionDescription(type: InterventionPlan['interventionType'], riskScore: number): string {
    const descriptions = {
