...
export interface SessionModification {
  type: 'CONTENT_DIFFICULTY' | 'VALIDATION_COMPLEXITY' | 'SESSION_DURATION' | 'BREAK_FREQUENCY' | 'SCAFFOLDING';
  description: string;
  value: string | number;
}
...
export class DifficultyAdapter {
  ...
  async adjustDifficulty(userId: string, currentLoad: number): Promise<DifficultyAdjustment> {
    ...
    const loadTolerance = userProfile?.loadTolerance || 65; // Default threshold
   ...
   */
  async recommendContentModification(sessionContext: SessionContext): Promise<ContentRecommendation> {
    ...
    const reasoning = this.generateContentRecommendationReasoning(currentLoad, performanceScore, errorRate);
  ...
  private generateAdjustmentReasoning(
    ...
    const personalizedNote = currentLoad > loadTolerance
  ...
  private generateContentRecommendationReasoning(
    ...
    if (currentLoad > 80) {
      ...
      reasoning += ', recommending familiar content with moderate scaffolding to reduce cognitive demand.';
    } else if (currentLoad >= 40) {
