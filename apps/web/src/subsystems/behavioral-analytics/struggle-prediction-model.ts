...
export interface PredictionResult {
  ...
  reasoning: {
    topFeatures: Array<{   // Features contributing most to prediction
      name: string;
...
export class StrugglePredictionModel {
  ...
  private predictRuleBased(featureVector: FeatureVector, dataQuality: number): PredictionResult {
    ...
    if (features.prerequisiteGapCount > 0.5) {
      ...
      riskFactors.push(`Missing ${(features.prerequisiteGapCount * 100).toFixed(0)}% of prerequisites`);
    }
    ...
    if (features.reviewLapseRate > 0.4) {
      ...
      riskFactors.push(`High rate of review failures (${(features.reviewLapseRate * 100).toFixed(0)}%)`);
    }
    ...
    if (features.retentionScore > 0.7) {
      protectiveFactors.push(`Strong retention score (${(features.retentionScore * 100).toFixed(0)}%)`);
    }
  ...
  private calculateBaselineProbability(features: FeatureVector): number {
    ...
    const weights = {
      ...
      sessionPerformanceScore: -0.10, // Negative weight
   ...
   */
  private predictLogisticRegression(featureVector: FeatureVector, dataQuality: number): PredictionResult {
  ...
  async getModelPerformance(): Promise<PerformanceMetrics> {
    ...
    const trend: 'IMPROVING' | 'STABLE' | 'DEGRADING' =
      currentMetrics.f1Score >= 0.75 ? 'IMPROVING' :
      currentMetrics.f1Score >= 0.65 ? 'STABLE' : 'DEGRADING';
