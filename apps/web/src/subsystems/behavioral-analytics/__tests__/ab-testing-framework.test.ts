...
describe('ABTestingFramework', () => {
  ...
  describe('createExperiment', () => {
    ...
    it('should set 2-week duration by default', async () => {
      ...
      expect(durationDays).toBe(14); // 2 weeks
    });
  });
  ...
  describe('assignUserToVariant', () => {
    ...
    it('should reject assignment when target user count reached', async () => {
      ...
      await expect(
        framework.assignUserToVariant('user-3', experimentId)
      ).rejects.toThrow('Experiment has reached target user count');
    });
  });
  ...
  describe('analyzeExperiment', () => {
    ...
    it('should detect insufficient duration (< 2 weeks)', async () => {
      ...
      expect(analysis.meetsRequirements.canConclude).toBe(false);
      expect(analysis.daysElapsed).toBe(7);
    });
    ...
    it('should detect no significant difference when variants are similar', async () => {
      ...
      expect(analysis.statistical?.recommendation).toContain('No statistically significant difference');
    });
    ...
    it('should calculate confidence intervals correctly', async () => {
      ...
      const trueDiff = 0.15;
      expect(analysis.statistical?.confidenceInterval.lower).toBeLessThanOrEqual(trueDiff);
      expect(analysis.statistical?.confidenceInterval.upper).toBeGreaterThanOrEqual(trueDiff);
    });
