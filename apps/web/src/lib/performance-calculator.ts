...
import type { Review, LearningObjective, Card } from '@/generated/prisma';
...
export class PerformanceCalculator {
  ...
  static calculateWeaknessScore(
    ...
    const retentionScore = this.calculateRetentionScore(reviews);
    ...
    const expectedTimeForMastery = this.estimateTimeForMastery(objective.complexity);
    const studyTimeFactor = Math.min(
      1.0,
      totalStudyTimeMs / expectedTimeForMastery
    );
  ...
  static async updateAllPerformanceMetrics(userId: string): Promise<void> {
    ...
    for (const objective of objectives) {
      await this.updateObjectivePerformance(userId, objective.id);
    }
  }
   ...
   */
  static async updateObjectivePerformance(
    userId: string,
    objectiveId: string
  ): Promise<void> {
    ...
    // Calculate metrics
    const retentionScore = this.calculateRetentionScore(reviews);
    const weaknessScore = this.calculateWeaknessScore(objective, reviews);
    ...
    if (todayReviews.length > 0) {
      ...
      const studyTimeMs = todayReviews.reduce(
   ...
   */
  private static estimateTimeForMastery(
  ...
  static async calculateSessionAnalytics(sessionId: string): Promise<{
    ...
    const totalReviewTimeMs = session.reviews.reduce(
      (sum, r) => sum + r.timeSpentMs,
      0
    );
    ...
    const totalObjectiveMs = validObjectiveMetrics.reduce(
      (sum, m) => sum + m.timeSpentMs,
      0
    );
    const averageObjectiveMs =
      validObjectiveMetrics.length > 0
        ? totalObjectiveMs / validObjectiveMetrics.length
        : 0;
    ...
    // Generate insights
    const insights = this.generateSessionInsights(
  ...
  private static generateSessionInsights(
    ...
    if (totalTimeDelta < 0) {
      ...
      insights.push(
        `You completed objectives ${percentFaster.toFixed(0)}% faster than estimated`
      );
    ...
    const highSelfAssessment = objectives.filter(
      (o) => o.selfAssessment && o.selfAssessment >= 4
    );
   ...
   */
  static async updatePerformanceFromSession(
    sessionId: string,
    userId: string
  ): Promise<void> {
    const session = await prisma.studySession.findUnique({
    ...
    for (const completion of objectiveCompletions) {
      ...
      const retentionScore = this.calculateRetentionScore(objectiveReviews);
      const masteryLevel = this.calculateMasteryLevel(
        retentionScore,
