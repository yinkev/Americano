...
export const updateUserProfileSchema = z.object({
  ...
  medicalSchool: z.string().max(200, 'Medical school name must be 200 characters or less').optional(),
...
export const createCourseSchema = z.object({
  ...
  color: z.string().regex(/^oklch\([\d\s._]+\)$/, 'Color must be in OKLCH format').optional(),
});

export const updateCourseSchema = z.object({
  name: z.string().min(1, 'Course name is required').max(100, 'Course name must be 100 characters or less').optional(),
  code: z.string().max(20, 'Course code must be 20 characters or less').optional(),
  term: z.string().max(50, 'Term must be 50 characters or less').optional(),
  color: z.string().regex(/^oklch\([\d\s._]+\)$/, 'Color must be in OKLCH format').optional(),
});
