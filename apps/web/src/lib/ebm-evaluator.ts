...
export class EBMEvaluator {
   ...
   */
  async evaluateSource(source: Source, specialty?: string): Promise<CredibilityRating> {
  ...
  async compareEvidence(conflictId: string, userId?: string): Promise<EBMComparison> {
    ...
    // Get source metadata (simplified for MVP - will enhance with actual Source model linkage)
    const sourceAType = conflict.sourceAFirstAid ? 'FIRST_AID' : 'LECTURE'
   ...
   */
  private determineEvidenceLevel(source: Partial<Source>): EvidenceLevel {
    // Check metadata for explicit evidence level
    const metadata = source.metadata as any
    if (metadata?.evidenceLevel) {
      return metadata.evidenceLevel as EvidenceLevel
    }

    // Default based on source type
    return SOURCE_TYPE_TO_EVIDENCE_LEVEL[source.type as SourceType]
  }
   ...
   */
  private calculateSpecialtyRelevance(source: Partial<Source>, specialty?: string): number {
  ...
  private calculatePeerReviewBonus(source: Partial<Source>): number {
    ...
    if (source.type === 'JOURNAL' || source.type === 'GUIDELINE') return 8 // Assumed peer-reviewed
    return 0
  }
