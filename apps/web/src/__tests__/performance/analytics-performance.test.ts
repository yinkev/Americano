...
>>>>>>> origin/main
  ...
  const generateMockMissions = (count: number) => {
    return Array.from({ length: count }, (_, i) => ({
      ...
      studySessions: [
        {
          reviews: [{ rating: 'GOOD' }, { rating: 'EASY' }],
        },
      ],
    }))
  }
  ...
  describe('Chart Data Preparation Performance', () => {
    it('should prepare chart data for 30 days within 500ms', async () => {
      ...
      const chartData = missions.map((m) => ({
        ...
        completionRate: m.status === MissionStatus.COMPLETED ? 1 : 0,
    ...
    it('should aggregate weekly data efficiently', async () => {
      ...
      // Group by week
      const weeklyData = missions.reduce(
        (acc, mission) => {
  ...
  describe('Database Query Optimization', () => {
    ...
    it('should efficiently aggregate with groupBy', async () => {
      ...
      ;(prisma.missionAnalytics.findMany as jest.Mock).mockResolvedValue(mockAnalytics)
      ...
      const avgScore = analytics.reduce((sum, a) => sum + a.avgSuccessScore, 0) / analytics.length
