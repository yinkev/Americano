...
test.describe('Story 4.2: Clinical Scenario Workflow', () => {
  ...
  test('Complete clinical reasoning workflow with all stages', async ({ page }) => {
    ...
    await expect(dialog.getByText(/diagnosis|differential|most likely/i)).toBeVisible({ timeout: 3000 });
    ...
    const feedbackPanel = page.locator('[data-testid="clinical-feedback"]').or(
      page.getByText(/competency|data gathering|diagnosis|management/i).locator('..')
    );
