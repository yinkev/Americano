...
test.describe('Story 4.1: Comprehension Validation Workflow', () => {
  ...
  test('Complete comprehension workflow with overconfidence calibration', async ({ page }) => {
    ...
    const calibrationPanel = page.locator('[data-testid="calibration-feedback"]').or(
      page
        .getByText(/calibration|confidence/i)
        .locator('..'), // fallback: find parent of calibration text
    )
    ...
    await reflectionTextarea.fill(
      'I thought I understood it well but I missed key details about pathophysiology.',
    )
    ...
    const completionIndicator = page
      .getByText(/complete|saved|great/i)
      .or(page.locator('[data-testid="study-session"]'))
