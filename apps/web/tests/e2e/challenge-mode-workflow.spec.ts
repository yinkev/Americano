...
test.describe('Story 4.3: Challenge Mode and Controlled Failure', () => {
  ...
  test('Complete challenge mode workflow with controlled failure', async ({ page }) => {
    ...
    const challengeDialog = page
      .locator('[role="dialog"]')
      .filter({ hasText: /challenge|test yourself/i })
    ...
    const feedbackPanel = page
      .locator('[data-testid="challenge-feedback"]')
      .or(page.getByText(/incorrect|not quite|almost|close/i).locator('..'))
    ...
    const notesTextarea = challengeDialog
      .locator('textarea')
      .filter({ hasText: /note|remember|why/i })
    if (await notesTextarea.isVisible({ timeout: 2000 })) {
      await notesTextarea.fill(
        'I confused this with a similar concept. Need to review the key difference.',
      )
    }
  ...
  test('View failure patterns on calibration dashboard', async ({ page }) => {
    ...
    const calibrationDashboard = page.locator('[data-testid="calibration-dashboard"]').or(
      page
        .getByText(/calibration|overconfident|underconfident/i)
        .first()
        .locator('..'),
    )
    ...
    const patternsList = page
      .locator('[data-testid="failure-patterns"]')
      .or(page.getByText(/pattern|systematic error|recurring/i).locator('..'))
